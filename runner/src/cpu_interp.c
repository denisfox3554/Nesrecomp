...
int cpu_interp_step(void) {
    static int count = 0;
    if (count < 50) {
        fprintf(stderr, "[CPU] step %d: PC=%04X op=%02X\n", count, cpu.PC, mem_read(cpu.PC));
        count++;
    }
    uint16_t pc = cpu.PC;
    uint8_t  op = RD(pc);
...
void cpu_interp_run(uint16_t entry) {
    ...
    int nested_count = 0;   /* счётчик вложенных вызовов */
    int max_nested = 1000;  /* максимум вложенных RTS/RTI */
    
    while (limit-- > 0 && nested_count < max_nested) {
        ...
        /* JSR — считаем как вложенный вызов */
        if (op == 0x20) {
            nested_count++;
            if (nested_count >= max_nested) {
                fprintf(stderr, "[WARN] cpu_interp_run: max nested calls reached\n");
                return;
            }
        }
    ...
    if (limit <= 0) {
        fprintf(stderr, "[WARN] cpu_interp_run: instruction limit reached\n");
    }
}
